import { TransfertCompteComponent } from './../transfert-compte/transfert-compte.component';
...
@Component({
  selector: 'app-show-compte-litige',
  templateUrl: './show-compte-litige.component.html',
  styleUrls: ['./show-compte-litige.component.css']
})
export class ShowCompteLitigeComponent implements OnInit {
  client!: any;
   ...
   createCompte(data: any){

      swalWithBootstrapButtons
              .fire({
                title: 'Attention !!!',
                text: `Vous êtes sur le point de créer un nouveau compte.`,
                icon: 'warning',
                showCancelButton: true,
    ...
    saveNewCompte(data:any){
      const body  ={
        client_id: data,
        solde:0
      }
       this.generalService.createClientCompte(body).subscribe({
       next: () => {

         setTimeout(() => {
           this.generalService.successEvent.emit(this.client);

         }, 300);
         this.sweetAlertService.toastSuccess('Compte creé avec succès', 3000);
       },
  ...
  const compte = Array.isArray(data)
    ...
    transfertSToCompteAtClient(data: any) {

     this.modalService.openModal(TransfertCompteComponent, data, 'modal-md modal-dialog-centered');
   }
   ...
   listAbonnementByCompte(data :any){
     ...
     getDataTransfertCompte(client:Client){
